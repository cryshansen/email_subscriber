<?php
/**
* THE WORKING COPY of Email Page Block form. 
* TODDO: refactor into one form with multiple displays (modal) via block tbd.
*/
 ...
 */
class EmailSubscribeForm extends FormBase {
...
// Inject it in your constructor:
 ...
 /**
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#prefix'] = '<div id="email-subscribe-form-wrapper">';
    $form['#suffix'] = '</div><div id="custom-email-subscribe-messages"></div>';
    ...
    $form['submit'] = [
      ...
      '#id' => 'email-subscribe-form-submit', // Unique ID for the submit button
      '#ajax' => [
        'callback' => '::ajaxSubmit',
        'event' => 'click',
      ],
    ...
    $form['#attached']['library'][] = 'email_subscriber/email_subscribe_form_email_styles';
    $form['#theme'] = 'email_subscribe_form';
  ...
  */
  ...
  public function ajaxSubmit(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::logger('email_subscriber')->info(" ajax  fired.");
    ...
    // ✅ Fix: define render array in a variable first.
    ...
    $response->addCommand(new HtmlCommand('#custom-email-subscribe-messages', $rendered_messages));




    \Drupal::logger('email_subscriber')->info(" ajax  after custom email-subscribe messages.");

    // Clear the form fields.
    $response->addCommand(new InvokeCommand(NULL, 'emailSubmitFormReset', ['email-subscribe-form']));
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::logger('email_subscriber')->info(" email subscribe submit fired.");
    try{
          
          $this->email_subscribe_insert_internal_db($values);
         
          /*
          * handle the hubspot data push here.
          */
          ...
          // use of brevo email opt in solution instead of this one below. 
          //Could use this like a path for brevo to listen like the opt in page controller. 
          //$this->sendthanksmail($values);
...
/**
  ...
  public function email_subscribe_brevo($values) {
   ...
   // Prepare the data to send. Sept 04  24 "listIds" = 26 Website Driven Newsletter Signup
   $data = [
      'updateEnabled' => true,
      'email' => $values['email'],
      "attributes" => [
    ...
    try {
      ...
      if ($statusCode == 200 || $statusCode == 201  || $statusCode == 204 ) {
        ...
        \Drupal::logger('email_subscriber')->info('Submission post success at Email newsletter form.{response}', [
          'response' => print_r($responseBody, TRUE),
        ]);
      }
      else {
       // $this->messenger()->addError($this->t('Subscription failed. Please try again later.'));
        \Drupal::logger('email_subscriber')->error('Subscription failed. Unexpected status code received from Brevo API: {status}', [
          '@status' => $statusCode,
        ]);
      }
    }
      ...
      \Drupal::logger('email_subscriber')->error('Submission remote post failed at Brevo Guzzle level Email Subscription form', [
        '@error' => $message,
      ]);
    }
  ...
  public function email_subscribe_send_hubspot_values($values){
      ...
      $data = [
        ...
        'context' => [
          ...
          'pageName' => 'EMTP Newsletter Signup Form',
      ...
      //$basic_auth_access_token = 'pat-nat-xxxxxx-xxxxx-xxxxx-xxxxx-xxxxxxxxx'; 
  
      $auth = 'Bearer ' . $basic_auth_access_token;	
      //$portalId = $config->get('email_subscriber.portal_id'); 
      // // portal is always the same
      $portalId =$config->get('email_subscriber.portal_id');
      $formId= $config->get('email_subscriber.form_id'); 
